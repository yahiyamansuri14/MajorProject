...
let savePost = async (req, res) => {
    ...
    let username = await getUserName(req.decoded.id)
    console.log(username)
    let newPost = new postModel({ textContent, likes, dislikes, user_id: req.decoded.id, fileUrl,username })
    newPost.save()
        .then(data => {
...
let dislikePost = async (req, res) => {
    ...
    let dislikes = dislike_person.length
    //if likes is 0 means its the first like , create an array of likes_person add this user id in it
